describe('Check the "Login" Page', () => {
  ...
  it('Customer login with empty email', () => {
    cy.get('body div.header-language-background p')
    cy.get('.skip-account').click()
    cy.get('#header-account > .links > ul > .last > a').click().should('have.text','Log In')
    cy.get('#pass').type('123456789').should('have.value','123456789')
    cy.get('#send2').click().should('have.text','Login')
    cy.get('#login-form > div > div.col-2.registered-users > div.content.fieldset > p.required')
